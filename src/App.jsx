...
function HomeBlurb({ animate, onAnimated, onNavigate }) {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => setCurrentThumb(c => c + 1), 140);
    return () => clearTimeout(t);
    ...
    <div className="home-blurb-parent" style={{ position: "relative", minHeight: "100vh", overflow: "hidden" }}>
        ...
        <div style={{ position: "absolute", inset: 0, zIndex: 10, background: "#000", display: "flex", alignItems: "center", justifyContent: "center" }}>
          {splashPhase === "thumbnails" && currentThumb < splashThumbs.length && (
            <img key={currentThumb} src={splashThumbs[currentThumb]} alt="" style={{
              ...
              animation: "thumbSnap 0.14s ease both",
            }} />
          )}
        </div>
      )}
      ...
      <div className="home-blurb-wrap" style={{ paddingTop: "24vh", minWidth: 720, width: 720, margin: "0 auto", position: "relative", zIndex: 2, opacity: showBlurb ? 1 : 0 }}>
        ...
        <style>{`@keyframes wordFade { from { opacity: 0; } to { opacity: 1; } }
          ...
          @keyframes thumbSnap { 0% { opacity: 0; transform: scale(1.02); } 15% { opacity: 1; transform: scale(1); } 85% { opacity: 1; } 100% { opacity: 0; } }
        `}</style>
      </div>
      {showPortrait && (
