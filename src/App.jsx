...
function HomeBlurb({ animate, onAnimated, onNavigate }) {
  ...
  const [delays] = useState(() => words.map(() => Math.random() * 8.5));
  ...
  const [splashThumbs] = useState(() => {
    const directors = PROJECTS.all.filter(p => p.role.includes("Director") || p.role === "Shot on iPhone");
    const editors = PROJECTS.all.filter(p => p.role.includes("Editor") && !p.role.includes("Director") && !p.title.includes("Google"));
    return [...directors, ...editors].map(p => p.splashThumb || p.thumb);
  });
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => setCurrentThumb(c => c + 1), 400);
    return () => clearTimeout(t);
  }, [splashPhase, currentThumb, splashThumbs.length]);
  useEffect(() => {
    if (splashPhase !== "pause") return;
    const t = setTimeout(() => setSplashPhase("done"), 200);
    return () => clearTimeout(t);
  }, [splashPhase]);
  useEffect(() => { if (animate) { const t = setTimeout(() => onAnimated(), 10000); return () => clearTimeout(t); } }, [animate, onAnimated]);
  ...
  const getWordStyle = (i, isBold, isName, link) => {
    ...
    if (animate && (splashPhase === "thumbnails" || splashPhase === "pause" || splashPhase === "done")) return { ...base, opacity: 0, animation: `wordFade 1.5s ease-in-out ${delays[i]}s forwards` };
    if (blurbState === "cascading-out") return { ...base, opacity: 1, animation: `wordFadeOut 0.8s ease-in-out ${delays[i] * (1.5/8.5)}s forwards` };
    if (blurbState === "hidden") return { ...base, opacity: 0 };
    if (blurbState === "cascading-in") return { ...base, opacity: 0, animation: `wordFade 1s ease-in-out ${delays[i] * (1.5/8.5)}s forwards` };
