...
function HomeBlurb({ animate, onAnimated, onNavigate }) {
  ...
  const [splashThumbs] = useState(() => {
    ...
    const editors = PROJECTS.all.filter(p => p.role.includes("Editor") && !p.role.includes("Director"));
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => setSplashPhase("blurb"), 500);
    return () => clearTimeout(t);
  }, [splashPhase]);
      ...
      {(splashPhase === "thumbnails" || splashPhase === "pause") && (
        <div style={{ position: "absolute", inset: 0, zIndex: 10, background: "#000", display: "flex", alignItems: "center", justifyContent: "center" }}>
          {splashPhase === "thumbnails" && currentThumb < splashThumbs.length && (
            <img key={currentThumb} src={splashThumbs[currentThumb]} alt="" style={{
              maxWidth: "55vw", maxHeight: "65vh", objectFit: "contain", display: "block",
              animation: "thumbSnap 0.4s ease both",
            }} />
          )}
        </div>
      )}
      ...
      <div className="home-blurb-wrap" style={{ paddingTop: "24vh", minWidth: 720, width: 720, margin: "0 auto", position: "relative", zIndex: 2, opacity: showBlurb ? 1 : 0 }}>
        <div className="home-blurb-text" style={{ fontSize: 26, fontWeight: 300, lineHeight: 1.65, letterSpacing: "0.01em", paddingRight: 40, textAlign: "justify" }}>
          {words.map((word, i) => {
            ...
            const handleClick = isName ? () => setShowPortrait(true) : link ? () => onNavigate(link) : undefined;
            const shouldAnimate = showBlurb && animate && splashPhase === "blurb";
            return (
              <span key={i} onClick={handleClick}
                style={{ display: "inline", color: isBold ? "#e8e4df" : "#807b74", fontWeight: isBold ? 400 : 300, opacity: shouldAnimate ? 0 : (showBlurb ? 1 : 0), animation: shouldAnimate ? `wordFade 1s ease-in-out ${delays[i]}s forwards` : "none", cursor: (isName || link) ? "pointer" : "default", transition: "opacity 0.3s" }}>
                {word}{" "}
              </span>
            );
          })}
        ...
        <style>{`@keyframes wordFade { from { opacity: 0; } to { opacity: 1; } }
          ...
          @keyframes thumbSnap { 0% { opacity: 0; transform: scale(1.02); } 10% { opacity: 1; transform: scale(1); } 80% { opacity: 1; } 100% { opacity: 0; } }
        `}</style>
      </div>
      {showPortrait && (
        <div onClick={() => setShowPortrait(false)} style={{
